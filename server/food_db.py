"""Nutritional database for Eat_Track.

Values are per 100 grams unless noted. Macros: calories (kcal), protein (g),
carbs (g), fat (g), fiber (g). Names use Romanian, without diacritics for the
lookup key; the parser normalizes user input the same way before matching.

To add a food, just append an entry to FOODS. `piece_grams` (optional) is the
default weight of one piece when the user writes "2 buc" / "o felie".
"""

# name: (calories, protein, carbs, fat, fiber)  -- per 100 g
FOODS = {
    # Mezeluri / lactate
    ...
    "ou": (155, 13.0, 1.1, 11.0, 0.0),  # ~50 g / bucata
...
def macros_for(name, grams):
    """Return macro dict for `grams` of `name` (name must be a FOODS key)."""
    cal, prot, carb, fat, fib = FOODS[name]
    factor = grams / 100.0
    return {
...
def scale_per100(per100, grams):
    """Scale a per-100g macro dict/tuple to `grams`.

    `per100` may be a (cal, prot, carb, fat, fib) tuple or a dict with the
    macro keys. Used for foods coming from external sources (OpenFoodFacts,
    LLM) that are not in the FOODS table.
    """
    if isinstance(per100, dict):
        cal = per100.get("calories", 0) or 0
        prot = per100.get("protein", 0) or 0
        carb = per100.get("carbs", 0) or 0
        fat = per100.get("fat", 0) or 0
        fib = per100.get("fiber", 0) or 0
    else:
